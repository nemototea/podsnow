...
| Home の下部タブバー（エピソード / 番組 / 設定） | Home 内の導線 | タブバーは入れず、ヘッダーにアプリ設定、上部の番組カードに番組設定への入口を置く | ルートを増やさず、対象の文脈に入口を置く（Issue #130） |
...
| E. 補助画面 | `src/app/show/index.tsx`、`src/features/show/AssetsSection.tsx`、`src/app/settings.tsx` |
...
書き出し完了 → 配信の準備 → コピー → エピソード一覧 → 設定 → 番組の設定
...
| 差 | 理由 |
|---|---|
| Home に下部タブバーが無い | §3。導線は設定アイコンと上部の番組カード |
...
| 一覧の右端が「›」ではなく「⋯」（操作メニュー） | 複製・音声を削除・エピソードを削除を残すため |
...
| 設計（#94 で実装した形） | #96 の形 |
|---|---|
| Home の番組名の上の「あなたの番組」、制作中カードの「制作中」 | 置かない。Issue #130 で制作中カード自体を廃止し、番組カードと全エピソードの一覧へ整理 |
...
| 素材画面のリード文、言語・テーマの「すぐに反映されます」 | 一つにまとめるか、削った |
