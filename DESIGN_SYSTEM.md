...
scheme、DB）は小文字の `podsnow` のまま変えない。
...
| 場所 | 見せる操作 | 見せないもの |
|---|---|---|
| Home | 未設定時の番組セットアップ、設定済みの番組カード、全エピソードの一覧、下部の「新しいエピソードを録る」、設定 | リスナー数、成長率、制作中の回の重複カード、下部タブバー |
...
| エピソード（書き出し） | アートワークつきプレーヤー（シーク、15 秒戻る、再生 / 一時停止、30 秒進む）、音の仕上げ、詳細、形式、書き出す、履歴 | 実際には連携しないサービス |
...
| 08 番組と素材 | `src/app/show/index.tsx`。番組、素材、新しいエピソードの構成、各ひな形の順。素材は見出しつきのカードから `show/assets.tsx` へ 1 タップ。アートワークは番組セクションに置く |
| 09 設定 | `src/app/settings.tsx` |

- **Home**: 上部に小さい `PodsNow.`。アートワークがあるときはロゴと設定の下に幅広い画像を置き、文字の無い帯で `bg` へフェードさせる。画像の視差移動は付けず、読み込み時のクロスフェードは「動きを減らす」で無効にする。【事実: Issue #133】番組設定済みなら、その下に番組名・著者・エピソード数を示す押下可能な番組カードを置き、番組設定への唯一の入口にする。未設定なら仮の番組名を見せず、「配信中の番組を取り込む」「新しく始める」のセットアップカードを置く。エピソードは制作中の 1 件を別カードにせず、すべて同じ一覧へ置く。状態（未録音・編集中・書き出し済み）はアイコンで示し、文字は読み上げに回す。復元した録音はボタンを覆わず、レイアウト内の通知で出す。
  音声を削除した回は「音声なし」と出し、0 秒と区別する。
  制作中の `episodes` と配信済みの `feed_episodes` は保存上分けたまま、Home では 1 一覧に見せる。明示リンクまたは GUID 完全一致した行だけを統合し、題名・話数だけではまとめない。書き出し済みファイル、RSS enclosure、ローカルのタイムラインのいずれかを再生できる行は再生操作を持つ。再生中は画面下部にミニプレーヤーを出し、画面を移っても再生を続ける。【事実: Issue #135】
- **プレーヤー**: シークは `adjustable` とし、内部の時間は `Smp`、表示時だけ時分秒へ変換する。アートワーク未登録時は番組のプレースホルダを使う。【事実: Issue #135】
